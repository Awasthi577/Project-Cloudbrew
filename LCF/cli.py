...
def runs_show(
    run_id: str = typer.Argument(..., help="Run identifier generated by cloudbrew create"),
):
    """Show stored metadata for a provisioning run."""
    data = provisioning_pipeline.get_run(run_id)
    if not data:
...
def runs_replay(
    ...
    """Replay a prior run deterministically using the saved final merged spec."""
    result = provisioning_pipeline.replay(run_id)
